from .gacha_service import GachaService


MMM_MM_MMMMMMMM_MESSAGE_MEDIUM = """
＿人人人人人人人人人人人人人人人人人＿
＞　萌萌萌・萌萌・萌萌萌萌萌萌萌萌　＜
￣Y^Y^Y^Y^Y^Y^Y^Y^Y^Y^Y^Y^Y^Y^Y^Y^Y￣
"""

MMM_MM_MMMMMMMM_MESSAGE_LARGE = """
​           
　　　■　　　■　　　
■■■■■■■■■■■
　　　■　　　■　　　
　　　■　　　■　　　
■■■■■■■■■■■
　■　　　　　　　■　
　　■　　　　　■　　
　　　■■■■■　　　
"""

MMM_MM_MMMMMMMM_JSON_URL = "https://hellomeg-assets.pages.dev/public/hellomegbot/mmm-mm-mmmmmmmm.json"


class MmmMmMmmmmmmmService(GachaService):
    """MmmMmMmmmmmmmコマンドのビジネスロジック"""
    
    def __init__(
        self,
        fever_minute: int = 0,
        ur_probability: float = 0.012345679,
        sr_probability: float = 0.18,
    ):
        super().__init__(
            message_medium=MMM_MM_MMMMMMMM_MESSAGE_MEDIUM,
            message_large=MMM_MM_MMMMMMMM_MESSAGE_LARGE,
            json_url=MMM_MM_MMMMMMMM_JSON_URL,
            fever_minute=fever_minute,
            ur_probability=ur_probability,
            sr_probability=sr_probability,
        )